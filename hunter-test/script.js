(() => {
  ...
  function calculate() {
    ...
    const coherentAnswers=HMA_QUESTIONS.filter((q,i)=>Object.keys(q.choices[state.answers[i]].scores).some(key=>key===sorted[0]||key===sorted[1])).length;
    const consistencyScore=Math.min(40,Math.round(spread*1.25+dominance*.55+(coherentAnswers/HMA_QUESTIONS.length)*12));
    ...
    const pool=HMA_ABILITIES[abilityKey]; const ability=hidden?{name:hidden.name,description:hidden.note,icon:"assets/icons/ability_placeholder.webp"}:pool[(consistencyScore+stats[sorted[0]])%pool.length]; const dept=hidden?{name:hidden.department,icon:"assets/icons/dept_placeholder.webp"}:HMA_DEPARTMENTS[sorted[0]];
  ...
  function renderResult(){const r=state.result,p=state.profile;show("result-screen");$("rank-image").src=HMA_RANKS.find(x=>x.name===r.rank).icon;$("rank-mark").textContent=r.rank.replace("급","");$("result-heading").textContent=r.ability.name;$("ability-description").textContent=r.ability.description;$("special-alert").hidden=!r.hidden;$("special-alert").textContent=r.hidden?"⚠ EX-CLASS HIDDEN PROFILE DETECTED":"";
    const fields=[["조회 대상",p.name],["성별",p.gender],["나이",`${p.age}세`],["등록번호",r.registration],["분류명",classification(p.age)],["성향 일관도",`${r.consistencyScore} / 40`],["능력 등급",r.rank],["초능력",r.ability.name],["배치 부서",r.dept.name],["직급",r.position],["위험도",r.risk]];$("result-fields").innerHTML=fields.map(([a,b])=>`<div><dt>${a}</dt><dd>${b}</dd></div>`).join("");$("result-note").textContent=r.note;const max=Math.max(...Object.values(r.stats));$("stat-chart").innerHTML=Object.entries(r.stats).map(([k,v])=>`<div class="stat-row"><span>${STATS[k]}</span><div class="stat-bar"><i style="width:${v/max*100}%"></i></div><b>${v}</b></div>`).join("");}
