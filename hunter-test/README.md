...
특성은 상위 두 스탯 조합과 능력 등급, 전체 스탯 분포를 함께 사용해 결정합니다. 등급이 높을수록 동일 계통 특성도 `잔향 → 초감각 → 기동식 → 전술식 → 영역식 → 권능 → 재해권능 → 세계식`으로 진화하며 효과와 범위가 커집니다. 전체 스탯 분포를 결정적 시드로 사용하므로 같은 등급이어도 분포가 다르면 다른 특성이 나올 수 있고, 완전히 같은 스탯 분포에는 항상 같은 특성이 나옵니다.
